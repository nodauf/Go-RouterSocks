...
func (s *Server) ServeConn(conn net.Conn) error {
	...
	_, authContext, err := s.authenticate(conn, bufConn)
	if err != nil {
	...
	_, request, err := NewRequest(bufConn)
	if err != nil {
...
func (s *Server) GetDest(conn net.Conn) ([]byte, []byte, string, *Request, error) {
	...
	// Authenticate the connection
	methodsByte, _, err := s.authenticate(conn, bufConn)
	if err != nil {
		err = fmt.Errorf("Failed to authenticate: %v", err)
		s.config.Logger.Printf("[ERR] socks: %v", err)
		return nil, nil, "", nil, err
	...
	receivedByteRequest, request, err := NewRequest(bufConn)
	secondBytes = append(secondBytes, receivedByteRequest...)
	if err != nil {
		if err == unrecognizedAddrType {
			if err := sendReply(conn, addrTypeNotSupported, nil); err != nil {
